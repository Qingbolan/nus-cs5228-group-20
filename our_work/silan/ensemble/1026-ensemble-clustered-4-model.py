...
from sklearn.ensemble import GradientBoostingRegressor, RandomForestRegressor  # 添加 RandomForestRegressor
...
def main():
    ...
    # 优化后的 RandomForestRegressor 的超参数
    rf_params = {
        'n_estimators': 200,        # Large number of trees for robust predictions
        'max_depth': 12,             # Moderate depth to prevent overfitting while capturing complexity
        'min_samples_split': 20,     # Matches with other models' min_samples settings
        'min_samples_leaf': 10,      # Conservative value to ensure stable leaves
        'max_features': 0.8,         # Similar to LightGBM's feature_fraction
        'bootstrap': True,           # Enable bootstrapping for better generalization
        'random_state': 42,          # Match seed with other models
        'n_jobs': -1,               # Use all available cores
        'max_samples': 0.7,         # Similar to LightGBM's bagging_fraction
    ...
    for cluster in range(len(cluster_info)):
        ...
        for fold, (train_index, val_index) in enumerate(kf.split(X_cluster), 1):
            ...
            model_rf.fit(X_train_processed, np.log1p(y_train))
            ...
            preds_rf = np.expm1(model_rf.predict(X_val_processed))  # Random Forest 的预测
            
            # 集成预测（平均）
            preds_ensemble = (preds_lgb + preds_gb + preds_cb + preds_rf) / 4.0  # 包含 Random Forest
            ...
            # SHAP 评估（以 LightGBM 为例）
            # explainer = shap.Explainer(model_lgb, X_train_processed)
            # shap_values = explainer(X_val_processed)
            # shap.summary_plot(shap_values, X_val_processed, show=False)
            # shap.save_html(f'shap_summary_cluster_{cluster}_fold_{fold}.html')
            # logging.info(f"SHAP summary saved for Cluster {cluster} Fold {fold}.")
            ...
            cluster_models.append({
                ...
                'random_forest': model_rf,  # 添加 Random Forest 模型
                'preprocessors': {
    ...
    # SHAP 全局解释（最后一个聚类的最后一个折叠）
    # if models:
    #     last_cluster_models = models[-1]
    #     if last_cluster_models:
    #         last_model_info = last_cluster_models[-1]
    #         model_lgb = last_model_info['lightgbm']
    #         preprocessors = last_model_info['preprocessors']
    #         X_processed, _, _, _, _ = preprocess_features(X, y, **preprocessors)
    #         explainer = shap.Explainer(model_lgb, X_processed)
    #         shap_values = explainer(X_processed)
    #         shap.summary_plot(shap_values, X_processed, show=False)
    #         shap.save_html('shap_global_summary.html')
    ...
    for cluster in range(len(cluster_info)):
        ...
        for model_dict in cluster_models:
            ...
            model_rf = model_dict['random_forest']  # 获取 Random Forest 模型
            ...
            try:
                ...
                preds_rf = np.expm1(model_rf.predict(X_test_processed))  # Random Forest 的预测
                
                preds = (preds_lgb + preds_gb + preds_cb + preds_rf) / 4.0  # 集成包含 Random Forest
    ...
    submission.to_csv('./submission_ensemble_clustered_optimized_4_model.csv', index=False)
    logging.info("Predictions complete. Submission file saved as 'submission_ensemble_clustered_optimized_4_model.csv'.")
