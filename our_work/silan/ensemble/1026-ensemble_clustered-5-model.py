...
# 添加随机森林的训练函数
def train_evaluate_rf(X_train, y_train, params):
...
def main():
    ...
    # Create KMeans clusters based on the optimal number of clusters
    ...
    # Initialize variables for out-of-fold (OOF) predictions and metrics
    oof_predictions = np.zeros(len(X))
    oof_mse = []
    oof_r2 = []
    
    # Initialize a list to store all models for each cluster
    models = []  # This will be a list of lists: models[cluster][fold]
    
    # Define hyperparameters for various models
    lgb_params = {
        'objective': 'regression',
        'metric': 'rmse',
        'boosting_type': 'gbdt',
        'verbosity': -1,  # Suppress LightGBM's per-iteration logs
        'seed': 42,
        'learning_rate': 0.05,
        'num_leaves': 31,
    ...
    cb_params = {
        ...
        'od_wait': 50,
        'random_seed': 42,
        'verbose': False  # Suppress CatBoost's logs
    }
    ...
    xgb_params = {
        ...
        'verbosity': 0  # Suppress XGBoost's logs
    }
    ...
    # Initialize a dictionary to collect feature importances from all models
    ...
    # Iterate over each cluster to train separate models
    for cluster in range(len(cluster_info)):
        ...
        # Initialize a list to store models for the current cluster
        cluster_models = []
        
        # Perform K-Fold cross-validation within the cluster
        for fold, (train_index, val_index) in enumerate(kf.split(X_cluster), 1):
            logging.info(f"Cluster {cluster} - Starting Fold {fold}")
            ...
            models_dict = {
                'lightgbm': train_evaluate_lightgbm(
                    X_train_processed, y_train, X_val_processed, y_val, lgb_params
                ),
                'xgboost': train_evaluate_xgboost(
                    X_train_processed, y_train, X_val_processed, y_val, xgb_params
                ),
                'catboost': CatBoostRegressor(**cb_params).fit(
                    X_train_processed, np.log1p(y_train)
                ),
                'random_forest': train_evaluate_rf(
                    X_train_processed, y_train, rf_params
                ),
                'gradient_boosting': GradientBoostingRegressor(
                    **gb_params
                ).fit(X_train_processed, np.log1p(y_train))
            }
            
            # Collect feature importances from each model
            for model_name, model in models_dict.items():
                importance_df = get_feature_importance(
                    model, X_train_processed.columns, model_name
                )
                feature_importance_dict[model_name].append(importance_df)
            
            # Make predictions on the validation set with each model
            predictions = {}
            for model_name, model in models_dict.items():
                if model_name == 'lightgbm':
                    predictions[model_name] = np.expm1(
                        model.predict(X_val_processed, num_iteration=model.best_iteration)
                    )
                else:
                    predictions[model_name] = np.expm1(model.predict(X_val_processed))
            
            # Ensemble predictions using predefined weights
            weights = {
                'lightgbm': 0.25,
                'xgboost': 0.25,
                'catboost': 0.2,
            ...
            preds_ensemble = sum(predictions[name] * weight for name, weight in weights.items())
            
            # Evaluate the ensemble predictions
            rmse = np.sqrt(mean_squared_error(y_val, preds_ensemble))
            r2 = r2_score(y_val, preds_ensemble)
            logging.info(f"Cluster {cluster} - Fold {fold} RMSE: {rmse:.4f}, R²: {r2:.4f}")
            
            # Store the trained models and their preprocessors for future use
        ...
        # Append all models of the current cluster to the main models list
    ...
    for model_name, importances in feature_importance_dict.items():
        if importances:
            combined_importance = pd.concat(importances)
            avg_importance = combined_importance.groupby('feature')['importance'].mean()
            logging.info(f"\nTop 10 important features from {model_name}:")
            logging.info(avg_importance.sort_values(ascending=False).head(10))
    
    elapsed_time = time.time() - start_time
    logging.info(f"\nTotal training time: {elapsed_time / 60:.2f} minutes")
    
    # Save all trained models and cluster information for future use
    ...
    # Since 'y' is not available for test data, use dummy_y_test for clustering
    dummy_y_test = np.zeros(len(X_test))
    test_clusters = predict_cluster(
        X_test, dummy_y_test, kmeans_model, models[0][0]['preprocessors'], 
    ...
    # Iterate over each cluster to make predictions
    for cluster in range(len(cluster_info)):
        ...
        # Aggregate predictions from all models within the cluster
        for model_dict in cluster_models:
            ...
            try:
                ...
                preds_xgb = np.expm1(model_xgb.predict(X_test_processed))
                preds_rf = np.expm1(model_rf.predict(X_test_processed))
                
                # Average predictions from all models
                preds = (preds_lgb + preds_cb + preds_gb + preds_xgb + preds_rf) / 5.0
    ...
    # Post-process the final predictions to ensure they are within valid price range
    ...
    submission.to_csv('./1026_ensemble_clustered_5_model.csv', index=False)
    logging.info("Predictions complete. Submission file saved as '1026_ensemble_clustered_5_model.csv'.")
