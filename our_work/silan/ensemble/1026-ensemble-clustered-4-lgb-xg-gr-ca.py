...
def create_price_clusters(X, y, n_clusters, features_for_clustering=['depreciation', 'coe', 'dereg_value']):
    price_percentiles = np.percentile(y, np.linspace(0, 100, n_clusters + 1))
    # 使用 n_clusters 作为中心数，不包括 0th percentile
    initial_centers = np.column_stack([
        np.log1p(price_percentiles[1:-1]),
        np.percentile(X[features_for_clustering], np.linspace(0, 100, n_clusters + 1))[1:-1]
    ])
...
def main():
    ...
    # 定义 XGBoostRegressor 的超参数
    xgb_params = {
        'n_estimators': 3000,               # 增加树的数量以提升模型稳定性
        'learning_rate': 0.05,              # 与其他模型一致
        'max_depth': 6,                     # 与 CatBoost 相似
        'subsample': 0.7,                   # 类似 bagging_fraction
        'colsample_bytree': 0.8,            # 类似 feature_fraction
        'objective': 'reg:squarederror',    # 回归任务
        'random_state': 42,
        'n_jobs': -1,                       # 使用所有的 CPU 核心加速训练
    }
    ...
    for cluster in range(len(cluster_info)):
        ...
        for fold, (train_index, val_index) in enumerate(kf.split(X_cluster), 1):
            ...
            model_xgb.fit(X_train_processed, np.log1p(y_train),
            ...
            preds_xgb = np.expm1(model_xgb.predict(X_val_processed))
            preds_svr = np.expm1(model_svr.predict(X_val_processed))  # SVR 的预测
            ...
            preds_ensemble = (
                weights['lightgbm'] * preds_lgb +
                weights['gradient_boosting'] * preds_gb +
                weights['catboost'] * preds_cb +
                weights['xgboost'] * preds_xgb +
                weights['svr'] * preds_svr
            )
            ...
            cluster_models.append({
                ...
                'xgboost': model_xgb,             # 添加 XGBoost 模型
                'svr': model_svr,                  # 添加 SVR 模型
                'preprocessors': {
    ...
    for cluster in range(len(cluster_info)):
        ...
        for model_dict in cluster_models:
            ...
            model_xgb = model_dict['xgboost']       # 获取 XGBoost 模型
            model_svr = model_dict['svr']           # 获取 SVR 模型
            ...
            try:
                ...
                preds_svr = np.expm1(model_svr.predict(X_test_processed))  # SVR 的预测
