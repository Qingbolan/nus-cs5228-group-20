...
def train_evaluate_models(X_train, y_train, X_val, y_val, lgb_params, cb_params, gb_params, price_range='medium'):
    """训练LightGBM、CatBoost和GradientBoosting模型并返回最优权重组合"""
    # 训练LightGBM
    ...
    # 根据价格区间调整权重搜索范围
    if price_range == 'high':
        w1_range = np.arange(0.2, 0.5, 0.05)  # LightGBM权重范围
        w2_range = np.arange(0.2, 0.4, 0.05)  # CatBoost权重范围
    elif price_range == 'low':
        w1_range = np.arange(0.4, 0.6, 0.05)  # LightGBM权重范围
        w2_range = np.arange(0.3, 0.5, 0.05)  # CatBoost权重范围
    else:
        w1_range = np.arange(0.3, 0.6, 0.05)  # LightGBM权重范围
        w2_range = np.arange(0.2, 0.4, 0.05)  # CatBoost权重范围
    ...
    for w1 in w1_range:
        for w2 in w2_range:
            w3 = 1 - w1 - w2
            if w3 < 0.05:  # 确保GradientBoosting至少有10%的权重
                continue
                
            weighted_pred = w1 * lgb_pred + w2 * cb_pred + w3 * gb_pred
    ...
    return lgb_model, cb_model, gb_model, best_weights
...
def main():
    ...
    for cluster in range(len(cluster_info)):
        ...
        for fold, (train_index, val_index) in enumerate(kf.split(X_cluster), 1):
            ...
            y_train, y_val = y_cluster.y_train, y_val = y_cluster.iloc[train_index], y_cluster.iloc[val_index]
            ...
            # 训练模型
            lgb_model, cb_model, gb_model, best_weights = train_evaluate_models(
            ...
            logging.info(f"Best weights for cluster {cluster}, fold {fold}: LGB={best_weights[0]:.2f}, "
                        f"CB={best_weights[1]:.2f}, GB={best_weights[2]:.2f}")
