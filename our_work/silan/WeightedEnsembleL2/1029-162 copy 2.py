...
def train_evaluate_xgboost(X_train, y_train, X_val, y_val, base_predictions, params):
    """
    使用其他模型的预测结果训练加权的XGBoost
    """
    ...
    # 创建DMatrix
    dtrain = xgb.DMatrix(X_train, label=np.log1p(y_train), weight=sample_weights)
    dval = xgb.DMatrix(X_val, label=np.log1p(y_val))
    
    # 微调XGBoost参数
    adjusted_params = params.copy()
    adjusted_params.update({
        'min_child_weight': 20,  # 增加以减少过拟合
        'subsample': 0.85,
...
def main():
    ...
    for cluster in range(len(cluster_info)):
        ...
        for fold, (train_index, val_index) in enumerate(kf.split(X_cluster), 1):
            ...
            preds_cb = np.expm1(model_cb.predict(X_train_processed))
            
            # 训练weighted XGBoost
            base_predictions = np.column_stack([preds_lgb, preds_gb, preds_cb])
            model_xgb, initial_weights = train_evaluate_xgboost(
                X_train_processed, 
                y_train, 
                X_val_processed, 
                y_val, 
            ...
            # 最终集成
            predictions_stack = np.column_stack([preds_lgb_val, preds_gb_val, preds_cb_val, preds_xgb_val])
    ...
    for cluster in range(len(cluster_info)):
        ...
        for model_dict in cluster_models:
            ...
            try:
                ...
                predictions_stack = np.column_stack([preds_lgb, preds_xgb, preds_gb, preds_cb])
