...
def main():
    ...
    X, y = load_and_preprocess_data('preprocessing/2024-10-30-silan/train_cleaned.csv')
    ...
    for cluster in range(len(cluster_info)):
        ...
        for fold, (train_index, val_index) in enumerate(kf.split(X_cluster), 1):
            ...
            # 组合预测结果用于CatBoost训练
            base_predictions = np.column_stack([preds_lgb, preds_gb, preds_xgb, preds_svr, preds_pytorch])
            # 训练CatBoost
            ...
            predictions_stack = np.column_stack([
                ...
                preds_svr_val, preds_pytorch_val, preds_cb_val
            ])
    ...
    # 预测测试数据
    X_test, _ = load_and_preprocess_data('preprocessing/2024-10-30-silan/test_cleaned.csv')
    ...
    for cluster in range(len(cluster_info)):
        ...
        for model_dict in cluster_models:
            ...
            try:
                ...
                predictions_stack = np.column_stack([
                    preds_lgb, preds_xgb, preds_gb,
                    preds_svr, preds_pytorch, preds_cb
                ])
