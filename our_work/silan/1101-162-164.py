...
def main():
    ...
    X, y = load_and_preprocess_data('l4_train.csv')
    ...
    with open('ensemble_clustered_models_l6.pkl', 'wb') as f:
        pickle.dump({
            'models': models,
    ...
    # 预测测试数据
    X_test, _ = load_and_preprocess_data('l4_test.csv')
    ...
    submission.to_csv('./submission_ensemble_clustered_optimized_l6.csv', index=False)
    logging.info("Predictions complete. Submission file saved as 'submission_ensemble_clustered_optimized_l6.csv'.")
    ...
    train_data = pd.read_csv('l4_test.csv')
    train_data['ref_price'] = oof_predictions  # 使用out-of-fold预测作为ref_price
    train_data.to_csv('l6_train.csv', index=False)
    logging.info("Training data with ref_price saved to l6_train.csv")
    
    # 处理测试集数据
    logging.info("\nProcessing test data...")
    test_data = pd.read_csv('l4_test.csv')
    test_data['ref_price'] = final_predictions  # 使用模型预测作为ref_price
    test_data.to_csv('l6_test.csv', index=False)
    logging.info("Test data with ref_price saved to l6_test.csv")
