...
def tune_parameters(X, y):
    """
    Main function to tune LightGBM parameters
    """
...
def main():
    ...
    for cluster in range(len(cluster_info)):
        ...
        # Get the best parameters for this cluster
        ...
        for fold, (train_index, val_index) in enumerate(kf.split(X_cluster), 1):
            ...
            model = train_evaluate_lightgbm(X_train_processed, y_train, X_val_processed, y_val, best_params)
    ...
    with open('lightgbm_clustered_models_tuned.pkl', 'wb') as f:
        pickle.dump({
            'models': models,
    ...
    logging.info("Predictions complete. Submission file saved as 'submission_lightgbm_tuned.csv'.")
