...
def create_parameter_grid():
    """Create a grid of parameters to search based on current settings"""
    param_grid = {
        'eta': [0.03, 0.05, 0.07],  # learning rate, centered around current 0.05
        'max_depth': [5, 6, 7],      # centered around current 6
        'min_child_weight': [1, 3, 5],
        'subsample': [0.7, 0.8, 0.9], # centered around current 0.8
        'colsample_bytree': [0.7, 0.8, 0.9], # centered around current 0.8
        'gamma': [0, 0.1, 0.2],      # expand from current 0
    }
    return param_grid

def evaluate_parameters(X, y, params, n_splits=5):
    """
    ...
    for train_idx, val_idx in kf.split(X):
        ...
        predictions = np.expm1(model.predict(dval))
...
def grid_search(X, y, param_grid):
    ...
    for combination in combinations:
        ...
        scores = evaluate_parameters(X, y, params)
        ...
        logging.info(f"RMSE: {scores['rmse_mean']:.4f} (±{scores['rmse_std']:.4f})")
        logging.info(f"R2: {scores['r2_mean']:.4f} (±{scores['r2_std']:.4f})")
        logging.info(f"Time taken: {elapsed_time:.2f} seconds")
        
        if scores['rmse_mean'] < best_score:
            best_score = scores['rmse_mean']
            best_params = params
            logging.info("New best parameters found!")
    
    return results, best_params

def tune_parameters(X, y):
    """
    Main function to tune XGBoost parameters
    """
    logging.info("Starting parameter tuning process...")
    param_grid = create_parameter_grid()
    results, best_params = grid_search(X, y, param_grid)
    
    sorted_results = sorted(results, key=lambda x: x['rmse_mean'])
    ...
    for i, result in enumerate(sorted_results[:5], 1):
        ...
        logging.info(f"RMSE: {result['rmse_mean']:.4f} (±{result['rmse_std']:.4f})")
        logging.info(f"R2: {result['r2_mean']:.4f} (±{result['r2_std']:.4f})")
...
def main():
    ...
    kmeans_model, price_clusters, cluster_info = create_price_clusters(X, y, n_clusters=optimal_clusters, 
    ...
    for cluster in range(len(cluster_info)):
        ...
        best_params = tune_parameters(X_cluster, y_cluster)
        logging.info(f"Best parameters for cluster {cluster}:")
        ...
        for fold, (train_index, val_index) in enumerate(kf.split(X_cluster), 1):
            ...
            model = train_evaluate_xgboost(X_train_processed, y_train, X_val_processed, y_val, best_params)
