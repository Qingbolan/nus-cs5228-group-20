...
def main():
    ...
    X, y = load_and_preprocess_data('preprocessing/2024-11-1-silan/train_cleaned.csv')
    ...
    # 预测测试数据
    X_test, _ = load_and_preprocess_data('preprocessing/2024-11-1-silan/test_cleaned.csv')
    ...
    submission.to_csv('./submission_ensemble_clustered_optimized_l2.csv', index=False)
    logging.info("Predictions complete. Submission file saved as 'submission_ensemble_clustered_optimized_l2.csv'.")
    ...
    train_data = pd.read_csv('preprocessing/2024-11-1-silan/train_cleaned.csv')
    train_data['ref_price'] = oof_predictions  # 使用out-of-fold预测作为ref_price
    train_data.to_csv('l4_train.csv', index=False)
    logging.info("Training data with ref_price saved to l4_train.csv")
    
    # 处理测试集数据
    logging.info("\nProcessing test data...")
    test_data = pd.read_csv('preprocessing/2024-11-1-silan/test_cleaned.csv')
    test_data['ref_price'] = final_predictions  # 使用模型预测作为ref_price
    test_data.to_csv('l4_test.csv', index=False)
    logging.info("Test data with ref_price saved to l4_test.csv")
