...
def main():
    ...
    with open('ensemble_clustered_models_l5.pkl', 'wb') as f:
        pickle.dump({
            'models': models,
    ...
    submission.to_csv('./submission_ensemble_clustered_optimized_l5.csv', index=False)
    logging.info("Predictions complete. Submission file saved as 'submission_ensemble_clustered_optimized_l5.csv'.")
    ...
    train_data = pd.read_csv('l3_train.csv')
    train_data['ref_price'] = oof_predictions  # 使用out-of-fold预测作为ref_price
    train_data.to_csv('l5_train.csv', index=False)
    logging.info("Training data with ref_price saved to l5_train.csv")
    
    # 处理测试集数据
    logging.info("\nProcessing test data...")
    test_data = pd.read_csv('l3_test.csv')
    test_data['ref_price'] = final_predictions  # 使用模型预测作为ref_price
    test_data.to_csv('l5_test.csv', index=False)
    logging.info("Test data with ref_price saved to l5_test.csv")
