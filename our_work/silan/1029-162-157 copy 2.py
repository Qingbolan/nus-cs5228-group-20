...
from sklearn.ensemble import RandomForestRegressor  # 添加这行
...
def main():
    ...
    X, y = load_and_preprocess_data('l1_train.csv')
    ...
    for cluster in range(len(cluster_info)):
        ...
        for fold, (train_index, val_index) in enumerate(kf.split(X_cluster), 1):
            ...
            model_rf = RandomForestRegressor(**rf_params)  # 改为RandomForest
            ...
            preds_rf = np.expm1(model_rf.predict(X_train_processed))  # 改为rf
            preds_xgb = np.expm1(model_xgb.predict(xgb.DMatrix(X_train_processed)))
            
            # 训练weighted CatBoost
            base_predictions = np.column_stack([preds_lgb, preds_rf, preds_xgb])  # 改为rf
            ...
            preds_rf_val = np.expm1(model_rf.predict(X_val_processed))  # 改为rf
            ...
            # 最终集成
            predictions_stack = np.column_stack([preds_lgb_val, preds_rf_val, preds_xgb_val, preds_cb_val])  # 改为rf
    ...
    with open('ensemble_clustered_models_cb_lgm_rf_xg.pkl', 'wb') as f:
        pickle.dump({
            'models': models,
    ...
    # 预测测试数据
    X_test, _ = load_and_preprocess_data('l1_test.csv')
    ...
    for cluster in range(len(cluster_info)):
        ...
        for model_dict in cluster_models:
            ...
            model_rf = model_dict['random_forest']  # 改名
            ...
            try:
                ...
                preds_rf = np.expm1(model_rf.predict(X_test_processed))  # 改为rf
                preds_cb = np.expm1(model_cb.predict(X_test_processed))
                
                predictions_stack = np.column_stack([preds_lgb, preds_rf, preds_xgb, preds_cb])  # 改为rf
    ...
    submission.to_csv('./submission_ensemble_clustered_cb_lgm_rf_xg.csv', index=False)
    logging.info("Predictions complete. Submission file saved as 'submission_ensemble_clustered_cb_lgm_rf_xg.csv'.")
