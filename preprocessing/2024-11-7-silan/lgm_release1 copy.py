...
def find_optimal_clusters(X, y, max_clusters=5, features_for_clustering=['depreciation', 'coe', 'dereg_value']):
    ...
    for n_clusters in range(2, max_clusters + 1):
        kmeans = KMeans(n_clusters=n_clusters, init='k-means++', n_init=3, random_state=42)
...
def create_price_clusters(X, y, n_clusters, features_for_clustering=['depreciation', 'vehicle_age', 'dereg_value','coe','power']):
    """
    Create clusters based only on the features specified, without using price (y) in clustering.
    
    Args:
        X: DataFrame containing feature columns
        y: Series of prices (used only for cluster analysis, not for clustering)
        n_clusters: Number of clusters to create
        features_for_clustering: List of feature names to use for clustering
    
    Returns:
        kmeans: Fitted KMeans model
        price_clusters: Array of cluster assignments
        cluster_df: DataFrame with cluster statistics
    """
    # Handle NaN values in clustering features
    cluster_features_df = pd.DataFrame(X[features_for_clustering])
    imputer = SimpleImputer(strategy='median')
    cluster_features_clean = imputer.fit_transform(cluster_features_df)
    
    # Initialize centers based on feature percentiles only
    initial_centers = np.zeros((n_clusters, len(features_for_clustering)))
    for i, feature in enumerate(features_for_clustering):
        feature_percentiles = np.percentile(cluster_features_clean[:, i], 
                                          np.linspace(0, 100, n_clusters))
        initial_centers[:, i] = feature_percentiles
    
    # Perform clustering using only the specified features
    kmeans = KMeans(n_clusters=n_clusters, init=initial_centers, n_init=3, random_state=42)
    price_clusters = kmeans.fit_predict(cluster_features_clean)
    
    # Analyze price distribution within each cluster
    cluster_info = []
    for cluster in range(n_clusters):
        cluster_prices = y[price_clusters == cluster]
        cluster_info.append({
...
def predict_cluster(X, y, kmeans_model, preprocessors, features_for_clustering=['depreciation', 'vehicle_age', 'dereg_value','coe','power']):
# (X, kmeans_model, preprocessors, features_for_clustering=['depreciation', 'vehicle_age', 'dereg_value', 'coe', 'power']):
    """
    使用预训练的 KMeans 模型对数据进行聚类预测，不考虑价格（y）。

    Args:
        X (pd.DataFrame): 输入特征数据。
        kmeans_model (KMeans): 预训练的 KMeans 模型。
        preprocessors (dict): 包含预处理器的字典（如缺失值填补器等）。
        features_for_clustering (list): 用于聚类的特征列表。

    Returns:
        np.ndarray: 聚类标签数组。
    """
    # 对输入数据进行预处理
    X_processed, _, _, _, _ = preprocess_features(X, None, **preprocessors)
    
    # 提取用于聚类的特征
    cluster_features_df = pd.DataFrame(X_processed[features_for_clustering])
    
    # 使用存储的 imputer 处理聚类特征中的缺失值
    cluster_features_clean = kmeans_model.feature_imputer.transform(cluster_features_df)
    
    # 仅使用清洗后的聚类特征进行预测，不再包含 y（price）
    cluster_features = cluster_features_clean
...
def main():
    ...
    features_for_clustering = ['depreciation', 'vehicle_age', 'dereg_value','coe','power']
