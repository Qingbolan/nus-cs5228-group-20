...
def impute_missing_values_optimized(data, target_columns, estimation_features, n_neighbors=5, z_threshold=3, min_features=3):
    """
    优化后的缺失值插补函数，支持多种填补策略、异常值处理和目标编码。
    ...
    - z_threshold: float, 用于异常值检测的Z-score阈值。
    ...
    try:
        ...
        if len(categorical_features) > 0:
            ...
            encoded_df = pd.DataFrame(encoded_features, columns=encoded_feature_names, index=data.index)
            data = pd.concat([data, encoded_df], axis=1)
            available_estimation_features = [feat for feat in available_estimation_features if feat not in categorical_features] + list(encoded_feature_names)

        # 目标编码函数
        # def target_encode(train, test, categorical_cols, target, smoothing=1):
        #     """
        #     基于训练集的目标变量进行目标编码，并应用到测试集。
        #     """
        #     encoding = {}
        #     for col in categorical_cols:
        #         averages = train.groupby(col)[target].mean()
        #         counts = train.groupby(col)[target].count()
        #         smooth = (averages * counts + smoothing * y.mean()) / (counts + smoothing)
        #         encoding[col] = smooth
        #         test[col] = test[col].map(smooth).fillna(y.mean())
        ...
        for target_column in available_target_columns:
            ...
            if len(numeric_features) > 0:
                ...
                if len(valid_data) > 0:
                    ...
                    selected_features = mi_series[mi_series > 0].sort_values(ascending=False).index.tolist()
                    selected_features = selected_features[:min_features] if len(selected_features) > min_features else selected_features
                else:
