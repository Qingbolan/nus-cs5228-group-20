...
def plot_mistake_distribution_with_categories(
    ...
    sns.histplot(data['mistake_abs'], bins=50, kde=True, color='skyblue', label='Mistake Distribution')
    ...
    for idx, center in enumerate(category_centers):
        plt.axvline(center, color=f'C{idx+1}', linestyle='--', label=f'Category {idx} Center')
    ...
    plt.title(f'Mistake Distribution with Category Centers - {model_name}')
    plt.xlabel('Absolute Mistake')
