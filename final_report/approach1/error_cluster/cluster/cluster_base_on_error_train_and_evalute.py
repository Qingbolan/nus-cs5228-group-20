...
# 导入新的分类器模块中的函数
from final_report.approach1.error_cluster.error_based_classifier import train_and_evaluate_classifier, predict_mistake_category
...
def predict_with_retrained_models(
    ...
    # 加载分类器
    classifier_data = pickle.load(open(classifier_path, 'rb'))
    classifier = classifier_data['classifier']
    label_encoder = classifier_data['label_encoder']
    scaler = classifier_data.get('scaler', None)
    num_imputer = classifier_data.get('num_imputer', None)
    ...
    for category_id in sorted(test_data['mistake_category'].unique()):
        ...
        if model_name == 'xgboost':
            ...
            preds = trained_model.predict(X_category_processed)
        elif model_name == 'catboost':
            preds = trained_model.predict(X_category_processed)
        elif model_name == 'gradient_boosting':
            preds = trained_model.predict(X_category_processed)
    ...
    if len(missing_indices) > 0:
        ...
        if model_name == 'xgboost':
            ...
            import lightgbm as lgb
            preds = trained_model.predict(X_missing_processed)
        elif model_name == 'catboost':
            preds = trained_model.predict(X_missing_processed)
        elif model_name == 'gradient_boosting':
            preds = trained_model.predict(X_missing_processed)
...
def main():
    ...
    model_name = 'gradient_boosting'  # 可以根据需要更改为 'lightgbm' 
    # catboost lightgbm xgboost 
    classifier_model = 'xgboost'  # 分类器模型名称
    ...
    # 使用新的 train_and_evaluate_classifier 函数
    classifier_result = train_and_evaluate_classifier(
    ...
    with open(classifier_save_path, 'wb') as f:
        pickle.dump({
            ...
            'scaler': classifier_result.get('scaler', None),
            'num_imputer': classifier_result.get('num_imputer', None)
        }, f)
    logging.info(f"分类器已保存到 {classifier_save_path}")
